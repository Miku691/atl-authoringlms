...
public class ImsTenantsController {
        ...
        @PutMapping("/{id}")
        public ResponseEntity<ApiResponse<ImsTenantsDto>> update(@PathVariable String id,
                        @RequestBody ImsTenantsDto dto) {
        ...
        @PutMapping("/{id}/verify-setup")
        public ResponseEntity<ApiResponse<Void>> verifySetup(
                        @PathVariable String id,
                        @RequestParam(required = false) com.atl.auth.enums.TenantType type) {
                service.verifySetup(id, type);
                return ResponseEntity.ok(
                                ApiResponse.<Void>builder()
                                                .status("SUCCESS")
                                                .statusCode(HttpStatus.OK.value())
                                                .message("Tenant setup verified and updated successfully")
                                                .build());
        }
}
