...
//        AtlRole superAdmin = roleRepo.findByRoleName(ApplicationConstant.TENANT_ADMIN_ROLE)
//                .orElseThrow(() -> new ResourceNotFoundException("Role Name", "SUPER_ADMIN"));
        ...
        return ApiResponse.<AtlSinginResponseDto>builder()
                        ...
                        .apiData(new AtlSinginResponseDto(saved.getUsername()
                                , maskedEmail))
                        .build();
        ...
        - creation process -> save (pending email verify -- first :: send otp --> on verify set to set up pending)
         */
    }
