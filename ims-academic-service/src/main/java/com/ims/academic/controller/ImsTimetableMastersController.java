...
public class ImsTimetableMastersController {
        ...
        @GetMapping("/offering/{offeringId}/tenant/{tenantId}")
        public ResponseEntity<ApiResponse<List<ImsTimetableMastersDto>>> getByOfferingIdAndTenantId(
                        @PathVariable String offeringId, @PathVariable String tenantId) {

                return ResponseEntity.ok(
                                ApiResponse.<List<ImsTimetableMastersDto>>builder()
                                                .status("SUCCESS")
                                                .statusCode(HttpStatus.OK.value())
                                                .message("Tenant Timetable Masters fetched successfully")
                                                .apiData(service.getByOfferingIdAndTenantId(offeringId, tenantId))
                                                .build());
        }
