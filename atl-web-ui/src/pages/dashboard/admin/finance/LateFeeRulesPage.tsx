...
const LateFeeRulesPage: React.FC = () => {
    ...
    if (isLoading) return <div className="p-8 text-center text-gray-400 font-medium">Synchronizing Rules...</div>;
        ...
        <div className="p-6 space-y-6 bg-gray-50/50 min-h-screen">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm font-medium">Define automated penalty structures for overdue institutional fees.</p>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {rules.map((rule) => (
                        <div key={rule.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group flex items-center justify-between border-l-4 border-l-indigo-500">
                            <div className="flex items-start gap-4">
                                <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                                    {rule.type === 'PERCENTAGE' ? <Percent size={20} /> : <Banknote size={20} />}
                                </div>
                                <div>
                                    <h3 className="font-bold text-gray-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{rule.name}</h3>
                                    ...
                                    <div className="mt-3 flex items-center gap-2 text-xs text-gray-500 font-medium italic">
                    ...
                    {rules.length === 0 && (
                        <div className="py-20 text-center bg-white rounded-2xl border-2 border-dashed border-gray-100">
                            ...
                            <h4 className="text-gray-500 font-bold text-xl">No policies defined</h4>
                            <p className="text-gray-400 text-sm mt-1">Institutional late fees are currently disabled.</p>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-bold text-gray-900 text-sm italic">Policy Overview</h3>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Policy Identifier *</label>
                                <input
                                    type="text"
                                    required
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold placeholder:font-normal"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Penalty Type *</label>
                                    <div className="flex p-1 bg-gray-100 rounded-xl">
                                        <button
                                            type="button"
                                            onClick={() => setFormData({ ...formData, type: 'FIXED' })}
                                            className={`flex-1 py-1.5 text-xs font-black rounded-lg transition-all ${formData.type === 'FIXED' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}
                                        >
                                            FIXED
                                        </button>
                                        <button
                                            ...
                                            className={`flex-1 py-1.5 text-xs font-black rounded-lg transition-all ${formData.type === 'PERCENTAGE' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}
                                        >
                                            PERCENT
                                        </button>
                                    </div>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Penalty Value *</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-8 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-black text-indigo-600"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Grace Period (Days) *</label>
                                <input
                                    type="number"
                                    required
                                    min="0"
                                    ...
                                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold"
                                />
                                <p className="text-[10px] text-gray-400 mt-2 italic">Policy becomes active X days after installment due date.</p>
                            </div>
                            ...
                            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-50">
                                ...
                                <button
                                    type="submit"
                                    className="flex items-center px-10 py-2.5 text-sm font-black text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all uppercase tracking-widest"
                                >
