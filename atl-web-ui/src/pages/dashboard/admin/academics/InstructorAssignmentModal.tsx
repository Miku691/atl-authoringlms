        ...
        <>
            <Modal
                ...
                footer={
                    <button
                        onClick={onClose}
                        className="px-8 py-2.5 bg-chrome text-content-primary font-bold rounded-xl hover:bg-chrome transition-colors"
                    >
                        Done
                    </button>
                }
            >
                <div className="flex flex-col h-full">
                    <p className="text-sm text-content-secondary mb-6">for {className}</p>
                        ...
                        <div className="bg-indigo-500/5 p-5 rounded-xl border border-indigo-500/10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                                <div className="lg:col-span-1">
                                    <label className="text-xs font-semibold text-content-secondary mb-1 block uppercase">Instructor</label>
                                    <select
                                        className="w-full p-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 bg-surface"
                                ...
                                <div className="lg:col-span-1">
                                    <label className="text-xs font-semibold text-content-secondary mb-1 block uppercase">Subject (Optional)</label>
                                    <select
                                        className="w-full p-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 bg-surface"
                                ...
                                <div className="lg:col-span-1">
                                    <label className="text-xs font-semibold text-content-secondary mb-1 block uppercase">Role</label>
                                    <select
                                        className="w-full p-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 bg-surface"
                                        value={role}
                                ...
                                <button
                                    ...
                                    className="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-lg shadow-indigo-600/20 dark:shadow-none flex items-center justify-center gap-2 h-[38px]"
                                >
                        ...
                        <div>
                            ...
                            {loading ? (
                                <div className="flex flex-col items-center justify-center py-12">
                                    ...
                                    <p className="text-content-secondary text-sm italic">Loading assignments...</p>
                                </div>
                            ) : assignments.length === 0 ? (
                                <div className="text-center py-12 border-2 border-dashed border-border rounded-2xl bg-chrome">
                                    <UserCheck className="w-10 h-10 mx-auto mb-3 text-gray-300" />
                                    <p className="text-content-secondary text-sm">No faculty assigned to this class yet.</p>
                                    <p className="text-xs text-content-muted mt-1">Assign an instructor to manage attendance and curriculum.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {assignments?.filter(a => a != null).map(a => (
                                        <div key={a.id} className="group relative flex items-center justify-between p-4 bg-surface border border-border rounded-2xl shadow-sm hover:border-indigo-500/30 hover:shadow-md transition-all animate-fadeIn">
                                            <div className="flex items-center gap-4">
                                                <div className="h-12 w-12 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-600 font-bold text-lg">
                                                    {a.instructorName?.[0] || '?'}
                                                </div>
                                                <div>
                                                    <p className="font-bold text-content-primary">{a.instructorName}</p>
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${a.role === 'PRIMARY' ? 'bg-indigo-500/10 text-indigo-600' : 'bg-chrome text-content-primary'
                                                            }`}>
                                                            {a.role}
                                                        </span>
