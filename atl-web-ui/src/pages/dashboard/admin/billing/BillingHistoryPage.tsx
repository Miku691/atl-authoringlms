        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1">Transaction records for your institution</p>
                </div>
                <button 
                    onClick={fetchHistory}
                    className="px-6 py-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-2xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2"
                >
                    Refresh Records
                </button>
            </div>
            ...
            {/* Table Control Panel */}
            <div className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <button className="flex-1 sm:flex-none px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-black text-slate-600 uppercase flex items-center justify-center gap-2 hover:bg-slate-100 transition-colors">
                                ...
                                <tr>
                                    <td colSpan={5} className="px-8 py-20 text-center">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Analyzing records...</p>
                                        </div>
                                    </td>
                                </tr>
                                        ...
                                        <tr key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-black text-slate-900 leading-tight">{item.planName}</p>
                                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-1">ID: {item.razorpayPaymentId || item.id.substring(0, 8)}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                                <p className="text-sm font-bold text-slate-700">{new Date(item.createdAt).toLocaleDateString([], { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                                                <p className="text-[10px] font-medium text-slate-400 mt-0.5">{new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                            </td>
                                            <td className="px-8 py-6">
                                                <p className="text-sm font-black text-slate-900">{item.currency} {new Intl.NumberFormat().format(item.amount)}</p>
                                            </td>
                                            <td className="px-8 py-6">
