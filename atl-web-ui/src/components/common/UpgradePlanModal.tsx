                ...
                >
                    ...
                    <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mt-1">Select a plan that scales with your growth</p>
                        </div>
                        ...
                        <div className="flex justify-center mb-10">
                            <div className="bg-slate-100 p-1 rounded-2xl flex items-center gap-1">
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${billingCycle === 'monthly' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    Monthly
                                </button>
                                <button
                                    onClick={() => setBillingCycle('yearly')}
                                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${billingCycle === 'yearly' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    Yearly
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {plans.map((plan) => {
                                    ...
                                    return (
                                        <div
                                            key={plan.id}
                                            className={`relative flex flex-col p-6 rounded-3xl border-2 transition-all duration-300 ${isHighlighted ? 'border-indigo-600 shadow-xl shadow-indigo-100' : 'border-slate-100 hover:border-slate-200'}`}
                                        >
                                            {isHighlighted && (
                                            ...
                                            <div className="mb-6">
                                                ...
                                                <div className="flex items-baseline gap-1 mt-2">
                                                    ...
                                                    <span className="text-slate-400 text-xs font-medium">/{billingCycle === 'yearly' ? 'year' : 'month'}</span>
                                                </div>
                                                <p className="text-xs text-slate-500 mt-2 min-h-[32px]">{plan.description}</p>
                                            </div>

                                            <ul className="space-y-3 mb-8 flex-1">
                                                <li className="flex items-start gap-2 text-xs font-semibold text-slate-700">
